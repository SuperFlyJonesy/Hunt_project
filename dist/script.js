...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function transitionToPage(targetUrl) {
        ...
        setTimeout(() => {
            let cleanTarget = targetUrl.replace(/\.html$/, '');
            if (cleanTarget === 'index' || cleanTarget === '/index') cleanTarget = '/';
    ...
    function performSearch() {
        ...
        hubSections.forEach(section => {
            ...
            tiles.forEach(tile => {
                ...
                const normalizedPath = baseHref.replace(/^\//, '').replace(/\.html$/, '');
                const aria = (tile.getAttribute('aria-label') || '').toLowerCase();
                const indexedKeywords = (PAGE_SEARCH_INDEX[baseHref] || PAGE_SEARCH_INDEX[normalizedPath + '.html'] || PAGE_SEARCH_INDEX['/' + normalizedPath] || '').toLowerCase();
                const combinedSearchCorpus = `${text} ${baseHref} ${normalizedPath} ${aria} ${indexedKeywords}`;
...
function initGlobalSocialLinks() {
    ...
    footers.forEach(footer => {
        ...
        socialBar.innerHTML = `
            ...
            <a href="path-join-us.html" class="hli-social-pill pill-community" title="Join our Community" aria-label="Join our Community"><span class="material-symbols-outlined" style="font-size:16px;">favorite</span><span>Community</span></a>
