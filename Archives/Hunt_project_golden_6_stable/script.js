                ...
                if (ticks >= 24) {
                    ...
                    actionPanel.innerHTML = `
                        <div style="margin-top: 10vh; text-align: center;">
                            <h2 style="color: #ffffff; font-size: 3rem; margin: 0 0 15px 0; font-weight: 900; letter-spacing: -1px; text-shadow: 0 4px 20px rgba(0,0,0,0.9);">One more person joined the community.</h2>
...
function initPrologue() {
    ...
    function playPrologueLoop() {
        ...
        // 3. Randomize Position accounting for text width
        const viewportWidth = window.innerWidth;
        const viewportHeight = window.innerHeight;
        const textWidth = currentStat.offsetWidth;
        const textHeight = currentStat.offsetHeight;

        // Calculate max allowed coordinates
        const maxLeft = viewportWidth - textWidth - 20;
        const maxTop = viewportHeight - textHeight - 20;

        // Assign random position
        currentStat.style.top = Math.max(10, Math.random() * maxTop) + 'px';
        currentStat.style.left = Math.max(10, Math.random() * maxLeft) + 'px';
